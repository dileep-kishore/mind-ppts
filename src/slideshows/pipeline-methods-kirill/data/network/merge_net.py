...
def get_uniq_nodes(data: '_coconut.typing.Sequence[dict]') -> '_coconut.typing.Sequence[dict]':
    return ((list)((_coconut_partial(unique, {}, 1, key=lambda x: x['id']))(reduce(_coconut.itertools.chain, map(_coconut.operator.itemgetter('nodes'), data)))))

@_coconut_tco
def get_link_key(link: 'dict') -> 'set':
    return _coconut_tail_call(frozenset, [link['source'], link['target']])

def get_overlap_links(data: '_coconut.typing.Sequence[dict]') -> '_coconut.typing.Sequence[dict]':
    all_links = (reduce(_coconut.operator.add, fmap(_coconut.operator.itemgetter('links'), data)))
    return ((list)((_coconut_partial(unique, {}, 1, key=get_link_key))(filter(lambda e1: ((lambda e3: True if e3 > 1 else False)((count)(filter(lambda e2: get_link_key(e1) == get_link_key(e2), all_links)))), all_links))))

def get_single_links(data: '_coconut.typing.Sequence[dict]', overlaps: '_coconut.typing.Sequence[dict]') -> '_coconut.typing.Sequence[dict]':
    all_links = (reduce(_coconut.operator.add, fmap(_coconut.operator.itemgetter('links'), data)))
    common_keys = fmap(get_link_key, overlaps)
    return ((list)(filter(lambda e1: get_link_key(e1) not in common_keys, all_links)))

class MyEncoder(json.JSONEncoder):
    @_coconut_tco
    def default(self, obj):
        if isinstance(obj, np.integer):
            return _coconut_tail_call(int, obj)
        elif isinstance(obj, np.floating):
            return _coconut_tail_call(float, obj)
        elif isinstance(obj, np.ndarray):
            return _coconut_tail_call(obj.tolist)
        else:
            return _coconut_tail_call(super(MyEncoder, self).default, obj)

def main(sources: '_coconut.typing.Sequence[str]') -> 'None':
    data = fmap(load_data, sources)
    uniq_nodes = get_uniq_nodes(data)
    overlap_links = (fmap(lambda ol: {**ol, **{'type': 'overlap'}}, get_overlap_links(data)))
    single_links = (fmap(lambda sl: {**sl, **{'type': 'single'}}, get_single_links(data, overlap_links)))
